...
if (!window.zeroOne && import.meta.env.DEV) {
  ...
  window.zeroOne = {
    getAppInfo: async () => ({ name: "ZERO ONE", version: "0.6.3-preview", platform: navigator.platform.toLowerCase().includes("mac") ? "darwin" : navigator.platform.toLowerCase().includes("linux") ? "linux" : "win32", packaged: false }),
    ...
    connectOpenZeroDesktop: async () => ({ settings: { ...previewSettings, hasOpenZeroToken: true }, hint: "ozd_preview" }),
