...
function SettingsView({ settings, appVersion, openZeroProbe, onSaved }: { settings: ZeroOneSettings; appVersion: string; openZeroProbe?: ServiceProbe; onSaved: (settings: ZeroOneSettings) => void }) {
  ...
  const selectedLocalProfileKnown = LOCAL_MODEL_PROFILES.some((profile) => profile.id === selectedLocalModel);
  ...
  const unloadLocalModels = async () => {
    setMessage("");
    try {
      const result = await localOpenZeroApi().unloadLocalOpenZeroModels?.({ all: true, model: selectedLocalModel });
      const unloaded = result?.unloaded || 0;
      await refreshLocalOpenZero();
      setMessage(unloaded ? `Unloaded ${unloaded} local model${unloaded === 1 ? "" : "s"} from memory.` : "No local model was loaded in memory.");
