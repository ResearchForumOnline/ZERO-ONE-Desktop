...
describe("desktop security contract", () => {
  ...
  it("pins an immutable, exact-architecture ZSEC payload and fails packaging closed", () => {
    expect(vendorLock).toMatchObject({
      ...
      consumer_version: packageJson.version,
    ...
    expect(beforePack).toContain("consumer_version");
    expect(beforePack).toContain("zsec-shield.lock.json");
    ...
    // Must not hard-code the app version (that caused package-windows CI failures on bumps).
    expect(beforePack).not.toMatch(/packageVersion !== "\d+\.\d+\.\d+"/);
    ...
    expect(vendorStager).toContain("package.json version");
